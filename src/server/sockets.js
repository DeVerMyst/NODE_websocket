const WebSocket = require('ws');
const { generateRandomNumbers } = require('../utils/utils');
const { logToFile } = require('../utils/logger');

let readingInterval;
let wss;

let isReading = false; 

const clients = [];

global.readingInterval = readingInterval; // Déclarer la variable readingInterval comme globale
global.clients = clients;

function configureSockets(server) {
    wss = new WebSocket.Server({ server });

    wss.on('connection', (ws) => {
        console.log('Client connecté');
        logToFile('Client connecté'); // Log client connecté

        clients.push(ws);

        ws.on('message', (message) => {
            try {
                const data = JSON.parse(message);
                handleClientAction(ws, data);
            } catch (error) {
                const errorMessage = 'Erreur lors de la manipulation du message du client : ' + error;
                console.error(errorMessage);
                logToFile(errorMessage); // Log erreur de manipulation du message client
            }
        });

        ws.on('close', () => {
            console.log('Client déconnecté');
            logToFile('Client déconnecté'); // Log client déconnecté
            removeClient(ws); // Retirer le client du tableau lors de la déconnexion
        });

        // Ajouter un gestionnaire d'erreur
        ws.on('error', (error) => {
            const errorMessage = 'Erreur WebSocket côté serveur : ' + error;
            console.error(errorMessage);
            logToFile(errorMessage); // Log erreur WebSocket côté serveur
        });
    });

    process.on('uncaughtException', (error) => {
        const errorMessage = 'Uncaught Exception : ' + error;
        console.error(errorMessage);
        logToFile(errorMessage); // Log uncaught exception
        process.exit(1); // Fermer le processus en cas d'exception non gérée
    });
}

function handleClientAction(ws, data) {
    switch (data.action) {
        case 'startReading':
            console.log('Client demande de démarrer la lecture');
            logToFile('Client demande de démarrer la lecture'); // Log client demande de démarrer la lecture
            startReading(ws);
            // Répondre au client avec un message de démarrage de la lecture réussi
            ws.send(JSON.stringify({ action: 'startReading', status: 'success' }));
            break;

        case 'connect':
            console.log('Client demande la connexion');
            logToFile('Client demande la connexion'); // Log client demande la connexion
            // Répondre au client avec un message de connexion réussie
            ws.send(JSON.stringify({ action: 'connect', status: 'success' }));
            break;

        case 'disconnect':
            console.log('Client demande la déconnexion');
            logToFile('Client demande la déconnexion'); // Log client demande la déconnexion
            // Répondre au client avec un message de déconnexion réussie
            ws.send(JSON.stringify({ action: 'disconnect', status: 'success' }));
            break;

        case 'stopReading':
            console.log('Client demande d\'arrêter la lecture');
            logToFile('Client demande d\'arrêter la lecture'); // Log client demande d'arrêter la lecture
            stopReading();
            // Répondre au client avec un message d'arrêt de la lecture réussi
            ws.send(JSON.stringify({ action: 'stopReading', status: 'success' }));
            break;

        default:
            console.log('Action non reconnue');
            logToFile('Action non reconnue'); // Log action non reconnue
            // Répondre au client avec un message d'action non reconnue
            ws.send(JSON.stringify({ action: data.action, status: 'error', message: 'Action non reconnue' }));
    }
}


function startReading(ws) {
    if (!readingInterval) {
        readingInterval = setInterval(() => {
            const randomNumbers = generateRandomNumbers(5); // Générer une liste de 5 nombres aléatoires
            const message = `La liste de nombres aléatoires est : ${randomNumbers.join(', ')}`;
            ws.send(JSON.stringify({ action: 'startReading', status: 'success', numbers: randomNumbers }));
            broadcastServerUpdate({ serverStatus: 'Reading' });
        }, 1000);
    }
}

function stopReading() {
    clearInterval(readingInterval);
    readingInterval = null;
    broadcastServerUpdate({ serverStatus: 'Stopped' });
}

function broadcastServerUpdate(data) {
    wss.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) {
            client.send(JSON.stringify({ action: 'serverUpdate', data }));
        }
    });
}

function removeClient(ws) {
    const index = clients.indexOf(ws);
    if (index !== -1) {
        clients.splice(index, 1);
    }
}

module.exports = {
    configureSockets,
};